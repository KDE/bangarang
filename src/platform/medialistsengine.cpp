...
void MediaListsEngine::run()
{
    ...
    if (m_mediaListProperties.engineArg() == "audio") {
        ...
        if (m_nepomukInited) {
            ...
            contextLRIs.clear();
            contextLRIs << "semantics://recent?video||limit=5||groupBy=actor";
            contextLRIs << "semantics://highest?video||limit=5||groupBy=actor";
            contextLRIs << "semantics://frequent?video||limit=5||groupBy=actor";
            mediaItem.fields["contextTitles"] = contextTitles;
            mediaItem.fields["contextLRIs"] = contextLRIs;
            mediaList << mediaItem;
            ...
            mediaItem.artwork = KIcon("view-media-artist");
            contextLRIs.clear();
            contextLRIs << "semantics://recent?video||limit=5||groupBy=director";
            contextLRIs << "semantics://highest?video||limit=5||groupBy=director";
            contextLRIs << "semantics://frequent?video||limit=5||groupBy=director";
            mediaItem.fields["contextTitles"] = contextTitles;
            mediaItem.fields["contextLRIs"] = contextLRIs;
            mediaList << mediaItem;
